...
////////////////////////////////////////////////////////////////////////////////
uint8_t flash_tx_data1[FLASH_BUF_SIZE]={0xFE,0x00};
uint8_t flash_tx_data2[FLASH_BUF_SIZE]={0xFE,0x01};
uint8_t flash_rx_data[FLASH_BUF_SIZE]={0};
////////////////////////////////////////////////////////////////////////////////
    ...
    static flash_status_t flash_erase_dual_bank(const uint32_t addr, const uint32_t size)
    {
        ...
        for ( uint8_t bank = 0; bank < eFLASH_BANK_NUM_OF; bank++ )
        {
            ...
            if ( bank_data[bank].size > 0 )
            {
                ...
                flash_erase.TypeErase = ((num_of_pages == FLASH_PAGE_NB) ? FLASH_TYPEERASE_MASS : FLASH_TYPEERASE_PAGES );
...
void flash_init_read(void)
{
    ...
    // if(flash_init()==eFLASH_OK);
    // {
    //     debug_tx2("Flash init success\r\n");
    // }
    // GPS_massage=0;
    flash_read(FLASH_ADDR_START,FLASH_BUF_SIZE, flash_rx_data);
    // GPS_massage=flash_rx_data[1];
    // flash_write(FLASH_ADDR_START,FLASH_BUF_SIZE, flash_tx_data1);
    // debug_tx2(flash_rx_data);
}
